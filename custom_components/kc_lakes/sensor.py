...
LAKE_SENSORS = [
    (KEY_WEATHER_LAST_UPDATE, "Weather Last Update", None, SensorDeviceClass.TIMESTAMP, SensorStateClass.MEASUREMENT, "mdi:clock-outline"),
    (KEY_AIR_TEMPERATURE, "Air Temperature", UnitOfTemperature.CELSIUS, SensorDeviceClass.TEMPERATURE, SensorStateClass.MEASUREMENT, "mdi:thermometer"),
    (KEY_WIND_SPEED, "Wind Speed", UnitOfSpeed.METERS_PER_SECOND, SensorDeviceClass.WIND_SPEED, SensorStateClass.MEASUREMENT, "mdi:weather-windy"),
    (KEY_WIND_DIRECTION, "Wind Direction", None, None, SensorStateClass.MEASUREMENT, "mdi:compass-outline"),
    (KEY_WATER_TEMPERATURE, "Water Temperature", UnitOfTemperature.CELSIUS, SensorDeviceClass.TEMPERATURE, SensorStateClass.MEASUREMENT, "mdi:water-thermometer"),
    (KEY_WATER_LAST_UPDATE, "Water Last Update", None, SensorDeviceClass.TIMESTAMP, SensorStateClass.MEASUREMENT, "mdi:clock-outline"),
    (KEY_BUOY_LATITUDE, "Buoy Latitude", DEGREE, None, SensorStateClass.MEASUREMENT_ANGLE, "mdi:latitude"),
    (KEY_BUOY_LONGITUDE, "Buoy Longitude", DEGREE, None, SensorStateClass.MEASUREMENT_ANGLE, "mdi:longitude"),
]
...
async def async_setup_entry(
    ...
    def _async_create_entities():
        ...
        for lake_name, lake_data in coordinator.data.items():
            ...
            for data_key, name_suffix, unit, dev_class, state_class, icon in LAKE_SENSORS:
                if data_key in lake_data: # Only create sensor if data exists
                     new_entities.append(
                         KCLakeSensor(coordinator, entry.entry_id, lake_name, data_key, name_suffix, unit, dev_class, state_class, icon)
                     )
                else:
                     _LOGGER.debug("Data key '%s' not found for lake '%s'", data_key, lake_name)

        if new_entities:
            _LOGGER.info("Adding %d King County Lake sensors", len(new_entities))
            async_add_entities(new_entities)
        else:
             _LOGGER.warning("No sensors could be created. Check API response and LAKE_SENSORS definition.")
...
class KCLakeSensor(CoordinatorEntity[KCLakesDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_device_info = {
            ...
            "model": "Lake Buoy",
            "via_device": (DOMAIN, config_entry_id), # Links device to the integration config entry
        }
